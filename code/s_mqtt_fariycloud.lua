    ...
    if  body then
        ...
        if result and type(tjsondata)=="table" then
            ...
            if status then
                ...
                _G.deviceModle = data["deviceModle"];
                _G.logFlag = data["logFlag"];
                _G.update_time = data["update_time"];
...
sys.taskInit(function()
    ...
    -- 四个继电器初始化
    _G.CT_RC1= gpio.setup(RC1, 0, gpio.PULLUP)
    _G.CT_RC2= gpio.setup(RC2, 0, gpio.PULLUP)
    _G.CT_RC3= gpio.setup(RC3, 0, gpio.PULLUP)
    _G.CT_RC4= gpio.setup(RC4, 0, gpio.PULLUP)

    -- LED初始化
    LED1 = gpio.setup(27, 0, gpio.PULLUP)
